...
- Correction review (`For Review` → `Complete` on approve / `For Review` → `For Correction` on return). Backend-authoritative contract `review_web_hr_emploc_correction(p_hr_emploc_id, p_decision, p_resolved_keys, p_remarks)` applied remotely via migration `20260609000000`. `canReviewCorrection` row capability normalized from `can_review_correction` (canonical key; stale `can_approve_correction` alias removed in OHM2026_1127). `ReviewCorrectionModal` wired in `HrEmplocDetailDrawer.tsx` with per-deficiency toggles, auto-derived approve/return decision, decision-preview block, remarks textarea, inline error display, and invalidation of `["hr-emploc-detail", id]`, `["hr-emploc-list"]`, `["hr-emploc-summary"]`. Remaining gap: `can_review_correction` must be present in the `get_web_hr_emploc_detail` row capability payload before the Review Correction button appears (OHM2026_1110-IMPL-1).
