-- Migration: 20260630000004_fix_coverage_review_artifacts_cte_scope
-- Created: 2026-06-30
-- Purpose: Fix CTE scope bug in _coverage_request_review_artifacts where pending_stores
--          CTE is referenced in a second SQL statement (count subquery) after its defining
--          WITH block is out of scope. Only surfaces when a sibling CG store has a pending
--          request, which triggers v_pending_exists = TRUE. Fix: pre-compute v_pending_count
--          in the same SELECT that defines the CTE.
--
-- Smoke Tests:
-- S1: reject_coverage_request('5fbe80b6-...') succeeds without 42P01 relation error
-- S2: approve_coverage_request('0fa4fbaf-...') succeeds and CG store is archived
-- S3: _coverage_request_review_artifacts returns conflict_report without error for
--     a request whose CG has a sibling pending request
...
BEGIN
  ...
  -- FIX (20260630000004): pre-compute v_pending_count in the same SELECT that defines the
  -- pending_stores CTE. The original code referenced pending_stores in a second SQL statement
  -- (inside jsonb_build_object), which is out of CTE scope and raises 42P01.
  DECLARE
  ...
  BEGIN
    ...
    pending_stores AS (
      ...
      WHERE pr.request_type IN (
        'create_coverage_group', 'add_store', 'remove_store',
    ...
    -- Both values computed in one statement while CTE is in scope
    SELECT
      EXISTS (
        SELECT 1 FROM public.stores s
        WHERE s.id = ANY(v_store_ids)
    ...
    INTO v_pending_exists, v_pending_count;
