-- OHM2026_0084A/B
-- Active duplicate import messages now include the existing active employee
-- name when resolvable.
--
-- Rollback-safe re-upload fix:
-- Review Import already marks rows as rollback_safe_employee when the matching
-- Plantilla row belongs to a rolled-back batch. The commit path previously let
-- those archived rollback rows fall into the inactive reconcile lookup. That
-- could make Approve mutate the old audit trace instead of creating the new
-- replacement row for the re-uploaded batch, and could surface generic commit
-- failure details to operators.
--
-- This follow-up keeps the blocking classifier unchanged and makes Approve
-- exclude rolled-back Plantilla rows from the inactive reconcile path. The
-- normal INSERT path then creates the new active row under the new batch while
-- the old rolled-back row remains archived/inactive for audit lineage.
--
-- Validation SQL checklist:
-- 1. Upload and approve a Plantilla batch.
-- 2. select public.rollback_plantilla_import_batch('<first_batch>'::uuid, 'validation rollback');
-- 3. Re-upload the same file under the same selected account and VCODE.
-- 4. Confirm Review Import rows are valid/flagged with rollback_safe_employee.
-- 5. select public.approve_plantilla_import_batch('<second_batch>'::uuid);
-- 6. Confirm result status = approved.
-- 7. Confirm one active employee per employee_no:
...
--    having count(*) <> 1;
-- 8. Confirm one active allocation per employee_no/account/VCODE:
--    select employee_no, account_id, upper(trim(vcode)), count(*)
--      from public.employee_store_allocations
--     where employee_no = '<emp_no>' and account_id = '<account_id>'::uuid and is_active
--     group by employee_no, account_id, upper(trim(vcode))
--    having count(*) <> 1;
-- 9. Confirm the first batch remains status = 'rolled_back' and its original
--    Plantilla row remains archived/inactive with source_baseline_import_batch_id
--    pointing to the first batch.
-- 10. Confirm an unrelated active same-account/different-VCODE allocation does
--     not block the rollback-safe re-upload.
