-- Migration: 20260706000012_fix_coverage_slots_single_row_and_position_inheritance.sql
-- Description: Fix _execute_approved_coverage_request() create_coverage_group branch (ohm#4h8w1qz6).
--
-- Fix 1 — coverage_slots per-store insertion bug:
--   The "remaining open slots" block inserted one coverage_slots row per store via
--   generate_series(v_slot_ordinal+1, v_store_count) whenever v_employees_converted = 0.
--   For an N-store pure-structure group this created N open slots instead of the
--   correct single demand slot (ADR-001: Coverage Group creation never derives
--   demand from store count). Replaced with a single fixed-row insert
--   (slot_ordinal = 1, status = 'open') when v_employees_converted = 0, regardless
--   of v_store_count.
--
-- Fix 2 — position_id inheritance from anchor store:
--   createCoverageGroup requests never carry position_id (the UI hides the field),
--   so coverage_groups.position_id was always NULL. Now resolved from the anchor
--   store's plantilla_slots before insert: prefer the anchor's occupied slot's
--   position_id (the active employee currently working that store); if none,
--   fall back to the anchor's open slot's position_id. If no eligible slot is
--   found, position_id remains NULL (does not block creation).
--
-- Out of scope (unchanged, verbatim from 20260702000003):
--   employees-converted branch (STEP 3 & 4 promotion loop), add_store, remove_store /
--   convert_roving_to_stationary, dissolve_coverage_group, merge_coverage_groups branches,
--   coverage_groups.required_headcount guard.
...
BEGIN
  ...
  IF p_request.request_type = 'create_coverage_group'::public.request_type THEN
    ...
    -- ── ohm#4h8w1qz6 Fix 2: resolve position_id from anchor store BEFORE insert.
    -- Prefer the anchor's occupied slot (active employee's position); fall back
    -- to the anchor's open slot. If neither exists, position_id stays NULL.
    v_anchor_store_id := (p_request.payload ->> 'anchor_store_id')::uuid;

    SELECT position_id INTO v_resolved_position_id
    FROM public.plantilla_slots
    WHERE store_id = v_anchor_store_id
      AND account_id = p_request.account_id
    ...
    v_resolved_position_id := COALESCE(p_request.position_id, v_resolved_position_id);
    ...
    -- (unchanged — employees-converted branch is out of scope for ohm#4h8w1qz6)
    v_slot_ordinal := 0;
    FOR v_pl IN (
    ...
    -- groups (v_employees_converted = 0), regardless of v_store_count. Previously
    -- generate_series(v_slot_ordinal+1, v_store_count) created one row per store.
...
COMMENT ON FUNCTION public._execute_approved_coverage_request(public.coverage_requests, uuid, text, text) IS
  'ohm#4h8w1qz6: create_coverage_group Fix 1 — exactly 1 open coverage_slots row for pure-structure '
  'groups (was 1 row per store via generate_series). Fix 2 — position_id inherited from anchor store''s '
  'plantilla_slots (prefer occupied slot, fallback to open slot; NULL if none found) instead of always '
  'NULL from p_request.position_id. All other branches (add_store, remove_store, '
  'convert_roving_to_stationary, dissolve_coverage_group, merge_coverage_groups) carried forward '
  'verbatim from 20260702000003.';
