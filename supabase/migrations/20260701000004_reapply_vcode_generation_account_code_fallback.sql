-- Migration: 20260701000004_reapply_vcode_generation_account_code_fallback
-- Created: 2026-07-01
-- Purpose: Re-apply generate_vcode_for_account account_code fallback fix — staging's live
--          function was still the pre-fix strict version despite migration 20260831000000
--          showing as "applied" in migration history, so VCODE creation was failing with
--          "account_code not set for account_id: %" for every account with a null account_code
--          (e.g. account THREE, id bb02474a-3928-4383-9eee-7a3393bdef4d).
--
-- Smoke Tests:
-- S1: SELECT public.generate_vcode_for_account('bb02474a-3928-4383-9eee-7a3393bdef4d');
--     Expected: returns a VC{initial}{group_code}_XXXX-style vcode, no exception raised.
-- S2: pg_get_functiondef of public.generate_vcode_for_account contains the fallback
--     branch (IF v_account_code IS NOT NULL AND TRIM(v_account_code) <> '') and no longer
--     unconditionally raises 'account_code not set for account_id: %'.
...
COMMENT ON FUNCTION public.generate_vcode_for_account(uuid) IS
  ...
  'OHM2026_0088: re-applies 20260831000000 fallback fix, which showed as applied in migration '
  'history on staging but whose function body never took effect (live function still raised '
  '"account_code not set for account_id: %" unconditionally). Idempotent CREATE OR REPLACE, '
  'no data mutation, no DDL change.';
