-- ============================================================
-- OHM2026_0075 (smoke-test fix) — Plantilla Import Rollback Executor
-- Migration: 20260911000002_fix_rollback_plantilla_import_batch_stores_type_column.sql
-- ============================================================
-- Root cause:
--   rollback_plantilla_import_batch (20260626000000) contained a SET clause
--   that referenced `type = (snap.r).type` in the batch-updated-stores block.
--   The `public.stores` table has NO `type` column. PostgreSQL fails at query
--   planning time with:
--     ERROR 42703: column "type" of relation "stores" does not exist
--   This was caught by the function's EXCEPTION block, which set
--   rollback_error_detail and returned {status: 'rollback_failed'}.
--   approve_plantilla_import_rollback then raised ROLLBACK_FAILED with the
--   error body, but the Flutter _humanizeRollbackError() had no pattern for
--   42703 / "type", so the user saw only the fallback:
--     "Rollback operation could not be completed."
--
-- Fix:
--   Remove `type = (snap.r).type` from the stores UPDATE SET clause.
--   No other logic, no other column, no other function is changed.
--
-- Scope:
--   Only rollback_plantilla_import_batch is modified.
--   Does NOT touch: Vacancy Import, MIKA, import upload/review/finalize,
--   approve_plantilla_import_rollback, request_plantilla_import_rollback,
--   governance triggers, listing RPCs.
--
-- Validation (run after applying):
--   V1  Confirm `type` no longer appears in function body:
--         SELECT prosrc FROM pg_proc
--          WHERE proname = 'rollback_plantilla_import_batch';
--         -- must NOT contain the substring "type = (snap.r).type"
--   V2  Direct smoke test with an approved+rollback_requested batch:
--         SELECT public.rollback_plantilla_import_batch('<batch_id>'::uuid, 'smoke test');
--   V3  Batch status after smoke test:
--         SELECT status, rolled_back_at, rollback_error_detail
--           FROM public.plantilla_import_batches WHERE id = '<batch_id>';
--         -- status = 'rolled_back', rollback_error_detail = NULL
-- ============================================================
...
COMMENT ON FUNCTION public.rollback_plantilla_import_batch(uuid, text) IS
  ...
  'OHM2026_0075 smoke-test fix: removed invalid stores.type SET reference (42703).';
