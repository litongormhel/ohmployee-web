-- Migration: 20260706000013_fix_anchor_position_id_resolution_source.sql
-- Description: Fix _execute_approved_coverage_request() create_coverage_group branch (ohm#4h8w1qz6 follow-up).
--
-- Root cause: 20260706000012's Fix 2 selected `position_id` directly from
-- `public.plantilla_slots`, but that table has no `position_id` column — only
-- a text `position` column. Every create_coverage_group execution therefore
-- raised 42703 "column position_id does not exist", discovered during staging
-- smoke testing before any test case could run.
--
-- Corrected resolution order (verified against live staging schema):
--   1. Occupied case: the anchor store's active plantilla row —
--      `plantilla.position_id` (plantilla DOES carry a uuid position_id FK;
--      plantilla_slots does not).
--   2. Open case fallback: `plantilla_slots.position` (text) is matched by
--      name to `positions.position_name` (text) to resolve `positions.id`.
--   3. If neither resolves, position_id remains NULL (does not block creation).
--
-- Out of scope (unchanged, verbatim from 20260706000012):
--   Fix 1 (single open coverage_slots row for pure-structure groups), all
--   other request_type branches (add_store, remove_store,
--   convert_roving_to_stationary, dissolve_coverage_group, merge_coverage_groups).
...
BEGIN
  ...
  IF p_request.request_type = 'create_coverage_group'::public.request_type THEN
    ...
    -- ── ohm#4h8w1qz6 Fix 2 (corrected): resolve position_id from anchor store
    -- BEFORE insert. plantilla_slots has NO position_id column — only text
    -- `position`. Prefer the anchor's active plantilla occupant's
...
COMMENT ON FUNCTION public._execute_approved_coverage_request(public.coverage_requests, uuid, text, text) IS
  ...
  'groups (was 1 row per store via generate_series). Fix 2 (corrected 20260706000013) — position_id '
  'inherited from anchor store: prefer plantilla.position_id of the active occupant, fallback to '
  'plantilla_slots.position matched against positions.position_name for an open slot; NULL if none '
  'found. All other branches (add_store, remove_store, convert_roving_to_stationary, '
  'dissolve_coverage_group, merge_coverage_groups) carried forward verbatim from 20260702000003.';
