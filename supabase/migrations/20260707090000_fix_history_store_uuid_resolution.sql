-- Create resolved activity log view
CREATE OR REPLACE VIEW public.v_employee_activity_log AS
SELECT
  id,
  emploc_no,
  vcode,
  ...
  activity_date,
  CASE
    WHEN activity_type = 'confirmed_onboard' AND description LIKE 'Coverage applicant confirmed onboard to %' AND metadata ? 'selected_stores' THEN
      'Coverage applicant confirmed onboard to ' || (
        SELECT string_agg(COALESCE(s.store_name, store->>'store_name'), ', ')
        FROM jsonb_array_elements(metadata->'selected_stores') AS store
        LEFT JOIN public.stores s ON s.id = (store->>'store_id')::uuid
      )
    ELSE description
  END AS description,
  performed_by,
  metadata,
  created_at
FROM public.employee_activity_log;

GRANT SELECT ON public.v_employee_activity_log TO authenticated, service_role;

-- Update onboarding RPC to write resolved names to description on future logs
...
BEGIN
  ...
  -- 9b. Aggregate resolved store names
  SELECT string_agg(store->>'store_name', ', ') INTO v_store_names
    FROM jsonb_array_elements(v_covered_json) AS store;
  ...
  INSERT INTO public.employee_activity_log (
    ...
    'confirmed_onboard',
    'Coverage applicant confirmed onboard to ' || COALESCE(v_store_names, ''),
    v_actor_name,
    jsonb_build_object(
