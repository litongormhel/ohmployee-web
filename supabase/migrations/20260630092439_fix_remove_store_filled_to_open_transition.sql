-- Migration: 20260630000005_fix_remove_store_filled_to_open_transition
-- Created: 2026-06-30
-- Prompt ID: ohm#9x2k6v7p
-- Purpose: Fix "Invalid vacancy status transition: Filled → Open" (23514) when
--          approving a Coverage Request → Remove Store.
--
-- Root cause (two-part):
--   1. fn_is_valid_vacancy_transition treats 'Filled' as a terminal state with no
--      exit path.  The Coverage Group Add Store execution (20260629060000 §3) sets
--      a vacancy to status='Filled', is_archived=true when a roving employee covers
--      a store.  Remove Store must reverse that — but the BEFORE UPDATE trigger
--      trg_enforce_vacancy_transition fires and raises 23514 before the UPDATE lands.
--
--   2. Even if the transition guard were absent, is_archived would remain TRUE after
--      the status→Open update, keeping the vacancy hidden from the Open tab.
--
-- Fix:
--   §1  fn_is_valid_vacancy_transition — add exactly one new allowed path:
--         Filled → Open
--       All other terminal protections (Filled→Closed, Closed→*, Rejected→*,
--       Archived→*) are unchanged.  The trigger trg_enforce_vacancy_transition is
--       NOT dropped or altered.
--
--   §2  _execute_approved_coverage_request — full CREATE OR REPLACE carrying
--       forward all fixes from 20260630000001 (v_vacancy_id UUID cast fix) with
--       one additional change in the remove_store vacancy-reopening block:
--         SET status='Open', is_archived=false, archived_at=NULL, archived_by=NULL
--       Only the remove_store path is changed.  add_store, create_coverage_group,
--       dissolve_coverage_group, merge, and convert branches are untouched.
--
-- Smoke Tests:
-- S1: Approve Remove Store for SM CALOOKOHAN — must complete without 23514
-- S2: VCTG2_0002 vacancy: status='Open', is_archived=false after approval
-- S3: CG membership for SM CALOOKOHAN is archived (coverage_group_stores.archived_at IS NOT NULL)
-- S4: Vacancy reuse: existing VCTG2_0002 row updated, no duplicate vacancy created
-- S5: Reject Remove Store still works — no change to rejection path
-- S6: fn_is_valid_vacancy_transition('Filled','Closed') still returns FALSE
-- S7: fn_is_valid_vacancy_transition('Filled','Pipeline') still returns FALSE
-- S8: fn_is_valid_vacancy_transition('Closed','Open') still returns FALSE
-- S9: fn_is_valid_vacancy_transition('Filled','Open') now returns TRUE

-- ════════════════════════════════════════════════════════════════════════════════
-- §1  Extend fn_is_valid_vacancy_transition — allow Filled → Open only
-- ════════════════════════════════════════════════════════════════════════════════
...
AS $$
  SELECT CASE
    ...
    -- This is the only exit from Filled.  All other Filled→X transitions remain blocked.
    WHEN p_old = 'Filled'           AND p_new = 'Open'                                     THEN TRUE
    -- Filled / Closed / Rejected / Archived are otherwise terminal.
...
-- ════════════════════════════════════════════════════════════════════════════════
-- §2  _execute_approved_coverage_request
--     Carry forward all fixes from 20260630000001 (UUID cast) plus:
--     — remove_store vacancy reopen now also resets is_archived / archived_at /
--       archived_by so the reopened vacancy surfaces in the Open tab.
--     All other branches are carried forward verbatim.
-- ════════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  THEN
    ...
    FOREACH v_store_id IN ARRAY v_removed_store_ids LOOP
      ...
      -- ── FIX ohm#4b9a7e2q: Use v_vacancy_id (uuid) to hold the existing vacancy id.
      -- ── FIX ohm#9x2k6v7p: Also reset is_archived/archived_at/archived_by so the
      --    vacancy surfaces in the Open tab.  The vacancy may be Filled+archived when
      --    a roving employee was covering this store (Add Store path sets Filled+archived).
      --    fn_is_valid_vacancy_transition now allows Filled→Open (§1 above).
