-- Migration: 20260707140000_fix_plantilla_import_middle_name_loss
-- Created: 2026-07-07
-- Prompt: ohm#7c2f9a1e (Fix Plantilla Import Name Parsing — Middle Name Loss)
-- Depends on: 20260707130000_plantilla_import_require_date_hired.sql
--
-- Root cause: approve_plantilla_import_batch Phase B employee aggregate query
--   used `(array_agg(middle_name ORDER BY row_number))[1]` WITHOUT the
--   FILTER (WHERE middle_name IS NOT NULL) guard that all other nullable
--   columns already use. For a roving employee whose first-ordered row had
--   middle_name = NULL (e.g. a store row with blank middle name while another
--   store row carried the correct value), the aggregate returned NULL and the
--   employee_name was stored without the middle name.
--
--   Additionally, the v_emp_name construction did not exclude "NA"/"N/A"
--   sentinel values from the middle name, unlike the active-employee conflict
--   name display that already had this guard.
--
-- Fix:
--   §1  submit_plantilla_baseline_import — no change needed. The staging INSERT
--       already stores v_middle correctly (NULLIF removes blanks; NA/N/A are
--       stored verbatim as-is, which is fine — the approve layer normalises them).
--
--   §2  approve_plantilla_import_batch — two surgical changes:
--       (a) Add FILTER (WHERE middle_name IS NOT NULL) to the middle_name
--           array_agg in the Phase B employee GROUP BY query, matching the
--           pattern already used for every other nullable enrichment column.
--       (b) Replace the raw v_emp_name assignment with a defensive builder that
--           also excludes "NA"/"N/A" sentinel strings, consistent with the
--           active-employee conflict display (lines 503-506 of the submit
--           function's v_active_emp_name computation).
--
-- Non-breaking guarantees:
--   - No schema changes (no ALTER TABLE).
--   - No other validation rules, conflict branches, slot wiring, HRCO
--     assignment, or rollback paths are modified.
--   - For employees without a middle name (NULL or blank), behaviour is
--     unchanged: v_emp_name = 'LAST, FIRST'.
--   - For employees with NA/N/A as their middle name, middle name is now
--     correctly omitted from employee_name (consistent with conflict display).
--   - For employees with a real middle name, it is now reliably preserved.
--   - All other fields carried over byte-for-byte from 20260707130000.
--
-- Regression surface: zero.
--   - employment_type / required_hc handling: untouched.
--   - submit_plantilla_baseline_import: untouched.
--   - RLS / grants / views: untouched.
--   - Flutter / Dart layer: no SQL-level dependency; Dart fix is in
--     store_import_service.dart (preview display only, does not affect DB).
...
COMMENT ON FUNCTION public.approve_plantilla_import_batch IS
  'ohm#7c2f9a1e: Fixed middle-name loss during Plantilla import approval. '
  'Added FILTER (WHERE middle_name IS NOT NULL) to Phase B middle_name array_agg, '
  'and replaced raw v_emp_name concat with a defensive builder that also strips '
  'NA/N/A sentinel values. All other logic byte-identical to 20260707130000.';
