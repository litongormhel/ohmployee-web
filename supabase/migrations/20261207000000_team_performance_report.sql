-- OHM2026_1138 Team Performance Report
-- RPCs: fn_team_performance_summary, fn_team_performance_detail
-- Both: SECURITY DEFINER, scope-gated, non-destructive, no workflow changes.
--
-- Scoring model (out of 100):
--   -5  MFR 90–94%
--   -10 MFR 80–89%
--   -20 MFR 70–79%
--   -30 MFR < 70%  (or 0 when required_hc = 0, not penalized)
--   -LEAST(aged_vacancies * 3, 20)  aged vacancies > 30 days
--   -LEAST(late_hr_updates * 5, 15) late plantilla updates > 7 days after separation
--   Score clamped [0, 100]
--
-- Severity:
--   90–100 Excellent
...
-- ── DROP existing versions to allow signature changes ──────────────────────
...
BEGIN
  ...
  WITH
  -- Scope guard: admin bypasses account filter
  scope AS (
    SELECT
      EXISTS (
        SELECT 1
        ...
        WHERE up.id = auth.uid() AND r.level >= 90
      ) AS is_admin
  ),
  allowed AS (
    SELECT get_my_allowed_accounts() AS ids
  ),
  ...
  base_accounts AS (
    ...
    WHERE
      ...
      AND (
        (SELECT is_admin FROM scope)
        OR a.account_name = ANY((SELECT ids FROM allowed))
      )
  ),

  -- Actual HC: Active + On Leave plantilla per account
  actual_hc AS (
    SELECT
      account_id,
      COUNT(*) AS hc
  ...
  -- Vacant HC: Open vacancies (slot-based fallback = vcode row count)
  vacant_hc AS (
    SELECT
      account_id,
      COUNT(*) AS hc
  ...
  -- Aged vacancies: open > 30 days since vacant_date or created_at
  aged_vac AS (
    SELECT
      account_id,
      COUNT(*) AS cnt
  ...
  -- Late HR updates: plantilla separation not updated within 7 days
  late_hr AS (
    SELECT
      account_id,
      COUNT(*) AS cnt
    FROM plantilla
    WHERE
      date_of_separation IS NOT NULL
      AND is_deleted = false
  ...
  -- Compute MFR and score per account
  scored AS (
    SELECT
      *,
      -- MFR: 0.0 when required = 0 (no data, not penalized)
      CASE
      ...
      GREATEST(0, LEAST(100,
        100
        -- MFR deduction (only penalize when there IS required HC)
        - CASE
...
-- Returns per-issue rows for one account (or all accounts in a group).
...
RETURNS TABLE (
  ...
  group_name        text,
  hrco_name         text,
  issue_category    text,  -- 'mfr' | 'aged_vacancy' | 'late_hr'
  issue_label       text,
  issue_detail      text,
  days_impacted     int,
  store_name        text,
...
BEGIN
  ...
  SELECT
    acct_id, acct_name, grp_id, grp_name, hrco,
    cat, lbl, dtl, days, store, emp, vc
  FROM aged_issues
  UNION ALL
  SELECT
    acct_id, acct_name, grp_id, grp_name, hrco,
