-- ============================================================
-- OHM2026_0065 - Plantilla Import Diff Preview Before Approval
-- Migration: 20260627000000_plantilla_import_diff_preview.sql
-- Depends on: 20260626000000_plantilla_import_rollback_v1.sql
-- ============================================================
-- Purpose
--   Adds a read-only approval diff RPC for unified Plantilla baseline import.
--   HA/SA can review create/update/replace/roving/blocked/skipped/flagged
--   impact before approving a pending batch.
--
-- Safety
--   - No commit, rollback, or validation logic is changed.
--   - SECURITY DEFINER with search_path pinned to public.
--   - Visibility matches existing preview access: full access or uploader.
--   - Snapshots are intentionally minimal operational fields only. No ATM,
--     government ID, or other sensitive personal fields are returned.
--
-- Validation checklist
--   V1 New employee row returns employee_create + allocation_create.
--   V2 Existing employee row returns employee_update and allocation_replace
--      when active account-scoped allocation exists.
--   V3 Multi-VCODE employee row includes roving.
--   V4 Blocked duplicate VCODE rows include blocked and are counted.
--   V5 Snapshots expose only operational store/employee/allocation fields.
-- ============================================================
...
BEGIN
  ...
  SELECT
    ...
    jsonb_build_object(
      ...
      'employees_to_create', count(*) FILTER (WHERE 'employee_create' = ANY(c.diff_categories)),
      'employees_to_update', count(*) FILTER (WHERE 'employee_update' = ANY(c.diff_categories)),
      'allocations_to_create', count(*) FILTER (WHERE 'allocation_create' = ANY(c.diff_categories)),
      'roving_employees', count(DISTINCT c.employee_no) FILTER (WHERE 'roving' = ANY(c.diff_categories)),
