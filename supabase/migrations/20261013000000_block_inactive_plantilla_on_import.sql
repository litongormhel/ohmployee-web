-- ============================================================
-- OHM2026_0083 — Block Inactive Plantilla Rows on Import
-- Migration: 20261013000000_block_inactive_plantilla_on_import.sql
-- Depends on:
--   20261009000000_align_plantilla_import_review_approve_validation.sql
--     (OHM2026_0079 — last canonical rewrite of both RPCs)
-- ============================================================
-- Purpose
-- -------
--   Changes the import validation classification for employees whose existing
--   plantilla row has status 'Inactive' or 'Rejected Deactivation' from
--   FLAGGED (reconcile path) to BLOCKED.
--
--   Before this migration, an inactive plantilla employee was flagged with
--   "existing_employee / will reconcile on commit" and approve_plantilla_import_batch
--   would reactivate (reconcile) the row. This was OHM2026_0071 behaviour.
--
--   This migration removes the reconcile path for inactive employees:
--     - Review Import (submit) now returns a BLOCKED row with
--       conflict_type = 'inactive_or_archived_existing_record'.
--     - Approve (commit) adds a defensive Check 3 that rejects any
--       valid/flagged row for which the employee still has an inactive
--       plantilla record at commit time (catches in-flight batches submitted
--       before this migration).
--     - The REACTIVATION path (v_inactive_id branch in Phase B) is
--       unreachable for batches submitted after this migration; it is left
--       intact for data-integrity safety but will not fire for new uploads.
--
-- Specific case resolved
-- ----------------------
--   SALVADOR JANET CARO | Emp 2020-02376 | VCS8LCG2_0003
--   Existing inactive plantilla → must be BLOCKED, not FLAGGED.
--
-- Business Rules Enforced (OHM2026_0083 additions)
-- --------------------------------------------------
--   Inactive plantilla employee  → BLOCKED  "inactive_or_archived_existing_record"
--   All other rules from OHM2026_0079: UNCHANGED
--
-- UI / data-contract
-- ------------------
--   conflict_type  : 'inactive_or_archived_existing_record'
--   msg            : 'Cannot import because matching employee, emploc, or
--                     VCode is inactive/archived. Restore/reactivate first
--                     or use a valid active record.'
--   Diff Preview   : row moves from Flagged Warnings → Blocked Rows.
--
-- Preserved
-- ---------
--   Active plantilla BLOCK                  : UNCHANGED (OHM2026_0079 P2)
--   Archived/deleted plantilla BLOCK        : UNCHANGED (OHM2026_0079 P3)
--   Rollback-safe plantilla ALLOW           : UNCHANGED (OHM2026_0079 P1)
--   Rollback-safe VCode ALLOW               : UNCHANGED
--   Archived VCode BLOCK                    : UNCHANGED
--   Archived Emploc BLOCK                   : UNCHANGED
--   Rollback-safe Emploc ALLOW              : UNCHANGED
--   Roving detection                        : UNCHANGED
--   HC mismatch sweep                       : UNCHANGED
--   Cross-account/group checks              : UNCHANGED
--   Phase A store upsert                    : UNCHANGED
--   Phase B new employee INSERT             : UNCHANGED
--   Phase B existing active employee UPDATE : UNCHANGED
--   Phase B Deactivated guard (skip)        : UNCHANGED
...
-- 20261009000000_align_plantilla_import_review_approve_validation.sql (OHM2026_0079).
--
-- Change vs OHM2026_0079 §1:
--   Priority 4 — employee conflict classification:
--     WAS: FLAG  (flag='existing_employee', reconcile on commit)
--     NOW: BLOCK (error, conflict_type='inactive_or_archived_existing_record')
-- ============================================================
...
COMMENT ON FUNCTION public.submit_plantilla_baseline_import IS
  'OHM2026_0083: Extends OHM2026_0079 (20261009000000) by changing Priority 4 of the '
  'employee conflict classification from FLAG (reconcile path) to BLOCK. '
  'Inactive/Rejected Deactivation plantilla employees are now blocked with '
  'conflict_type=inactive_or_archived_existing_record. They must be manually '
  'restored/reactivated before re-import. '
  'All other rules from OHM2026_0079 are preserved: '
  '(1) Active plantilla employee → BLOCKED (active_employee). '
  '(2) Archived/deleted non-rollback-safe → BLOCKED (archived_employee). '
  '(3) Rollback-safe plantilla → ALLOWED (rollback_safe_employee). '
  '(4) Inactive/Rejected Deactivation → BLOCKED (inactive_or_archived_existing_record). '
  '(5) Deleted hr_emploc without rollback-safe ESA → BLOCKED (archived_emploc). '
  '(6) Rollback-safe via ESA → ALLOWED (rollback_safe_emploc). '
  '(7) Archived/non-reusable VCode → BLOCKED (archived_vcode). '
  '(8) Rollback-safe VCode → ALLOWED (rollback_safe_vcode). '
...
-- 20261009000000_align_plantilla_import_review_approve_validation.sql (OHM2026_0079).
--
-- Change vs OHM2026_0079 §2:
--   Added Check 3 in defensive revalidation block:
--     Inactive/Rejected-Deactivation plantilla employee in valid/flagged rows.
--     Catches in-flight batches submitted before OHM2026_0083 where inactive
--     employees were flagged (not blocked).
-- ============================================================
...
COMMENT ON FUNCTION public.approve_plantilla_import_batch IS
  'OHM2026_0083: Extends OHM2026_0079 (20261009000000) with defensive Check 3. '
  'Before Phase A (store upsert), the function now also checks for Inactive / '
  'Rejected Deactivation plantilla employees in valid/flagged rows and raises '
  'REVALIDATION_FAILED (ERRCODE 55000) if any are found. This catches in-flight '
  'batches submitted before OHM2026_0083 where such rows were flagged (reconcile '
  'path) rather than blocked. '
  'The Phase B inactive reconcile path (v_inactive_id branch) is preserved for '
  'data-integrity safety but is unreachable for batches submitted after '
  'OHM2026_0083. '
  'All other logic from OHM2026_0079 is preserved: Phase A store upsert, '
  'Phase B new employee INSERT, existing active employee UPDATE, Deactivated skip, '
  'roving override, slot occupancy wire, VCODE resolution guard, audit logging, '
