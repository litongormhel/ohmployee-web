-- Migration: 20260629070000_fix_coverage_dissolve_province_id
-- Created: 2026-06-29
-- Purpose: Fix runtime 42703 error (column "province_id" does not exist) in
--          _execute_approved_coverage_request by replacing two stale
--          SELECT…province_id,area_name FROM stores queries (remove_store and
--          dissolve_coverage_group vacancy-reopening loops) with the correct
--          LEFT JOIN accounts pattern already running on staging.
--
-- Smoke Tests:
-- S1: Approve a dissolve_coverage_group request — must complete without 42703 error
-- S2: Approve a remove_store request — must reopen vacancy without 42703 error
-- S3: Approve an add_store request — must still work (unaffected branch)
-- S4: Reject a coverage request — must still work (calls different path)
-- S5: No stores.province_id or stores.area_name referenced in updated function
