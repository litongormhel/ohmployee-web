-- Migration: 20260706000014_remove_dead_position_id_coalesce_carryover.sql
-- Description: Cleanup _execute_approved_coverage_request() create_coverage_group branch (ohm#8m2k5vqr).
--
-- Dead code removal only — no behavior change:
--   `v_resolved_position_id := COALESCE(p_request.position_id, v_resolved_position_id);` was a
--   012 carryover. `createCoverageGroup` requests never populate `p_request.position_id` (the UI
--   hides the field per OHM2026_0097), so `p_request.position_id` is always NULL for this branch
--   and the COALESCE was a no-op — it always evaluated to `v_resolved_position_id`. Removed to
--   avoid implying `p_request.position_id` is ever consulted here.
--
-- Out of scope (unchanged, verbatim from 20260706000013):
--   Fix 1 (single open coverage_slots row for pure-structure groups), anchor position_id
--   resolution order (occupied plantilla.position_id, then plantilla_slots.position matched
--   against positions.position_name), all other request_type branches (add_store, remove_store,
...
COMMENT ON FUNCTION public._execute_approved_coverage_request(public.coverage_requests, uuid, text, text) IS
  ...
  'found. ohm#8m2k5vqr (20260706000014): removed dead 012-carryover COALESCE(p_request.position_id, '
  '...) — createCoverageGroup never populates p_request.position_id, so it was always a no-op. '
  'All other branches (add_store, remove_store, convert_roving_to_stationary, '
