-- ============================================================================
-- ohm#25f6a6ra — Fix Remove Store: Correct Semantics, Vacancy Reopening,
--                Deployment History
-- Migration: 20261232000000_fix_remove_store_correct_semantics.sql
--
-- Root Causes:
--   1. store_ids semantics were INVERTED in all previous remove_store migrations:
--      payload.store_ids was treated as "stores to KEEP", but the Flutter UI
--      sends the stores selected for REMOVAL (stores to REMOVE).
--      This caused the wrong stores to be archived and wrong vacancy reopening.
--
--   2. No vacancy slot reopening was performed for removed stores.
--      After coverage removal, the store's slot must be reopened (or created)
--      with vacancy_reason 'Coverage Store Removed', reusing existing VCode.
--
--   3. No deployment history / activity log for removed stores.
--
-- Fix:
--   payload.store_ids  = stores to REMOVE  (source of truth)
--   v_removed_ids      = payload.store_ids
--   v_remaining_ids    = active stores NOT in payload.store_ids
--   v_remaining_count  = cardinality of v_remaining_ids
--
--   For each removed store:
--     1. Archive coverage_group_stores edge
--     2. Reopen or create vacancy in `vacancies` (by VCode from stores.vcode)
--     3. Reopen or create plantilla_slot  (slot-first architecture)
--     4. Write slot_history entry
--     5. Deactivate ESA row for employee (imported) / update store link (live)
--
--   Stationary auto-conversion (remaining_count = 1) continues to work,
--   using v_remaining_ids[1] — the CORRECT remaining store.
--
-- Supersedes: remove_store branch of
--   20261230000000_normalize_and_execute_imported_roving_groups.sql
--   20261231000000_fix_remove_store_auto_convert.sql
-- ============================================================================

BEGIN;

-- ── §1. Add COVERAGE_STORE_REMOVED to slot_reason_codes ─────────────────────
INSERT INTO public.slot_reason_codes (code, label, description, sort_order)
VALUES (
  'COVERAGE_STORE_REMOVED',
  'Coverage Store Removed',
  'Store was removed from a coverage group; slot reopened or created.',
  80
)
ON CONFLICT (code) DO NOTHING;

-- ── §2. Re-implement Execution Engine ────────────────────────────────────────
--
--   SEMANTICS CORRECTION (ohm#25f6a6ra):
--     payload.store_ids  = stores the user selected TO REMOVE
--     v_remaining_ids    = active stores NOT selected for removal
--     v_remaining_count  = how many stores remain after removal
--
--   This fixes the inverted logic in previous migrations where store_ids was
--   treated as "stores to KEEP" (inverse of the UI contract).
--
...
COMMENT ON FUNCTION public._execute_approved_coverage_request(public.coverage_requests, uuid, text, text) IS
  'ohm#25f6a6ra: Fix remove_store semantics — payload.store_ids = stores to REMOVE (not keep). '
  ...
  'Supersedes 20261230000000 and 20261231000000 remove_store branches.';


-- ── §3. Update _validate_coverage_request_payload — correct remove_store semantics comment
--   The validation logic is already correct (checks store_ids are active CG members),
--   which matches the new semantics (store_ids = stores to REMOVE = must be active members).
--   No change to validation logic needed; update comment only.
COMMENT ON FUNCTION public._validate_coverage_request_payload(
  public.request_type, jsonb, uuid, uuid, text, uuid, uuid, uuid
) IS
  'ohm#25f6a6ra: Validates coverage request payloads. '
  'For remove_store: payload.store_ids = stores to REMOVE (corrected semantics). '
  'All listed store_ids must be active members of the target coverage group.';
