-- Migration: 20262906000002_fix_sla_requester_auth_guard
-- Created: 2026-06-29
-- Purpose: In check_closure_request_sla and check_hc_request_sla, _rnw_auth_uid()
--          can return a stale auth UUID (profile row exists but the auth.users row
--          was deleted). The SA/HA approver loops are guarded by JOIN auth.users,
--          but the requester notify path was not. Add EXISTS guard on v_req_auth
--          before passing it to notify_user() to prevent FK violation on notifications.
--
-- Smoke Tests:
-- S1: SELECT public.check_hc_request_sla(); returns void with no FK error
-- S2: SELECT public.check_closure_request_sla(); returns void (consistent guard)
-- ============================================================


-- ============================================================
-- §1  Fix check_closure_request_sla — auth.users guard on v_req_auth
-- ============================================================
  ...
  LOOP
    ...
    IF NOT public.notification_sla_exists('vacancy_closure', r.id::TEXT, lvl) THEN
      IF lvl = 2 THEN
        ...
        END LOOP;
        -- Resolve and guard requester auth UUID against stale/deleted auth users
...
-- ============================================================
-- §2  Fix check_hc_request_sla — same auth.users guard on v_req_auth
-- ============================================================
