-- ============================================================
-- OHM2026_1136 — Fix Import Vacancy audit_action enum failures
-- Migration: 20260730000001_fix_import_vacancy_audit_action_enum.sql
-- Depends on: 20260730000000_import_vacancy_migration_v1.sql
-- ============================================================
-- Problem
--   approve_vacancy_import and reject_vacancy_import insert into
--   audit_logs with enum labels that do not exist in audit_action:
--     reject path:          'REJECTION'    → invalid
--     approve failure path: 'COMMIT_FAILED' → invalid
--
-- Fix (no new enum labels added)
--     'REJECTION'    → 'UPDATE'   (batch status transition to rejected)
--     'COMMIT_FAILED' → 'UPDATE'  (batch status transition to commit_failed)
--     'APPROVAL'     → kept as-is (already valid)
-- ============================================================


-- ============================================================
-- §1  approve_vacancy_import — fix COMMIT_FAILED audit_action
-- ============================================================
...
EXCEPTION WHEN others THEN
  ...
  -- Uses UPDATE (batch status transition) — 'COMMIT_FAILED' is not a valid audit_action enum label.
  UPDATE public.vacancy_import_batches
...
COMMENT ON FUNCTION public.approve_vacancy_import IS
  ...
  'OHM2026_1136: commit-failure audit log now uses UPDATE (COMMIT_FAILED was invalid).';


-- ============================================================
-- §2  reject_vacancy_import — fix REJECTION audit_action
-- ============================================================
...
BEGIN
  ...
  -- Uses UPDATE (batch status transition) — 'REJECTION' is not a valid audit_action enum label.
  INSERT INTO public.audit_logs(actor_id, module, action, record_id, new_data)
  VALUES (v_uid, 'vacancy_import_batches', 'UPDATE', p_batch_id,
    jsonb_build_object('event', 'rejected', 'reason', p_reason));
...
COMMENT ON FUNCTION public.reject_vacancy_import IS
  'Reject RPC for the one-time Import Vacancy migration (OHM2026_1135). '
  'RBAC: Head Admin / Super Admin only. '
  'OHM2026_1136: audit log now uses UPDATE (REJECTION was an invalid enum label).';
