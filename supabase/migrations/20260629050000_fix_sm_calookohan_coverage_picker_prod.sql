-- Migration: 20260629050000_fix_sm_calookohan_coverage_picker_prod
-- Created: 2026-06-29
-- Ticket: ohm#8r6xk2m9
-- Target: PRODUCTION (rwxelulyapjgaarlwkus)
-- Purpose: Fix SM CALOOKOHAN missing from Coverage Request → Add Store → Available Stores.
--
-- Root Cause (confirmed by database investigation):
--   1. Migrations 20260629030000 and 20260629040000 were applied to STAGING only.
--      Production (rwxelulyapjgaarlwkus) did not receive these migrations.
--   2. fn_trg_auto_create_vacancy_slot on production still resolves store_id via
--      stores.vcode lookup only. When VCTG2_0002 vacancy was created, stores.vcode for
--      SM CALOOKOHAN was NULL → plantilla_slots.store_id was set to NULL.
--   3. get_coverage_group_eligible_stores only includes stores where:
--        EXISTS (plantilla_slots WHERE store_id = s.id AND slot_status <> 'closed')
--        OR EXISTS (employee_store_allocations WHERE store_id = s.id AND is_active = true)
--      With plantilla_slots.store_id = NULL, SM CALOOKOHAN fails both predicates.
--   4. stores.vcode for SM CALOOKOHAN is also NULL (never backfilled).
--
-- Evidence:
--   - stores.id = fb83f507-25e9-4a97-8be1-6bdd30ed622d (SM CALOOKOHAN, THREE, active)
--     stores.vcode = NULL
--   - vacancies.vcode = VCTG2_0002, store_id = fb83f507..., status = Open, is_archived = false
--   - plantilla_slots.legacy_vcode = VCTG2_0002, store_id = NULL, slot_status = open
--
-- Production orphan slots (all repaired by §3 data repair):
--   VCAG2_0017  open      SM Biñan     (vacancy.store_id = 7b0f3929...)
--   VCAG2_0018  pipeline  SM Biñan     (vacancy.store_id = 7b0f3929...)
--   VCAG2_0019  pipeline  SM Bagulin   (vacancy.store_id = e709b3c4...)
--   VCTG2_0002  open      SM CALOOKOHAN (vacancy.store_id = fb83f507...)  ← TARGET
--
-- Fix sections:
--   §1  Update fn_trg_auto_create_vacancy_slot — prioritize NEW.store_id (from ohm#9x4p2k7q)
--   §2  Update create_plantilla_slot_from_request — sync stores.vcode (from ohm#9x4p2k7q)
--   §3  Data repair — backfill plantilla_slots.store_id from vacancies + stores.vcode backfill
--   §4  Update get_coverage_group_eligible_stores — add active vacancy footprint guard
--   §5  Verification queries (in comments)
--
-- Regression rules:
--   - Preserves ADR-001 (slot-first architecture)
--   - No destructive SQL (UPDATE with guards only)
--   - No frontend changes
--   - No VCode overwrites (WHERE vcode IS NULL guard)
--   - Archived/inactive stores remain excluded
-- ============================================================
...
--     Source: 20260629030000_fix_vacancy_store_mismatch_coverage.sql §1
--     Change: Prioritizes NEW.store_id before falling back to stores.vcode lookup
-- ============================================================
...
COMMENT ON FUNCTION public.fn_trg_auto_create_vacancy_slot() IS
  'OHM2026_0074 / ohm#9x4p2k7q / ohm#8r6xk2m9 — Auto-creates plantilla_slots for newly opened/pipeline vacancies. '
  'Prioritizes NEW.store_id and falls back to stores.vcode lookup only when NEW.store_id IS NULL. '
  'Applied to production in 20260629050000.';


-- ============================================================
-- §2  Update Request Completion: create_plantilla_slot_from_request
--     Source: 20260629030000_fix_vacancy_store_mismatch_coverage.sql §2
--     Change: Syncs newly generated VCodes to stores.vcode; includes VCODE reuse logic
-- ============================================================
...
COMMENT ON FUNCTION public.create_plantilla_slot_from_request(uuid) IS
  ...
  'Updates stores.vcode when generating a new VCode (ohm#9x4p2k7q / ohm#8r6xk2m9). '
  'Applied to production in 20260629050000.';


-- ============================================================
-- §3  Safe Data Repair for Production Orphan Slots
--     Repairs all 4 active orphan slots (store_id IS NULL) identified in production.
--     Strategy: backfill from vacancies.store_id via legacy_vcode match (unambiguous).
--     Affected VCodes: VCAG2_0017, VCAG2_0018, VCAG2_0019, VCTG2_0002 (PRIMARY TARGET)
-- ============================================================

-- §3A. Backfill plantilla_slots.store_id from vacancies.store_id (via legacy_vcode match)
-- Safe: only updates rows where store_id IS NULL and vacancy has a definitive store_id.
...
-- §3B. Fallback: Backfill plantilla_slots.store_id from stores.vcode match
-- Covers any remaining orphans where vacancy also has store_id = NULL but stores.vcode matches.
UPDATE public.plantilla_slots ps
SET store_id   = s.id,
    updated_at = now()
FROM public.stores s
...
-- §3C. Backfill stores.vcode from active non-pool vacancies (safe, unambiguous only)
-- Only updates stores where: vcode IS NULL AND exactly 1 active VCode exists for that store.
-- This restores SM CALOOKOHAN stores.vcode = VCTG2_0002 (and any other safe cases).
...
-- ============================================================
-- §4  Update get_coverage_group_eligible_stores — Add Active Vacancy Footprint
--     Task 8 defense: If a store has an active open/pipeline vacancy but its
--     plantilla_slots.store_id is NULL (e.g., legacy orphan from before §1 fix),
--     the store still appears in the picker via its vacancy footprint.
--     Preserves all existing predicates (plantilla_slot and ESA checks).
--     Preserves archived/inactive exclusions.
--     Does not include filled/closed/deleted/pool vacancies.
-- ============================================================
...
COMMENT ON FUNCTION public.get_coverage_group_eligible_stores(uuid) IS
  'OHM2026_0102 / ohm#8r6xk2m9: Coverage Group store picker. '
  'Returns only active, non-archived stores under an active, non-archived account. '
  'Eligibility: active plantilla slot (store_id match) OR active ESA OR active open/pipeline '
  'vacancy (defense against legacy orphan slots with store_id=NULL). '
  'already_grouped_code ignores archived groups and edges. '
  'Applied to production in 20260629050000.';
...
-- ============================================================
-- §5  Verification Queries (in comments — run post-apply to confirm)
-- ============================================================
--
-- A. Store linkage (SM CALOOKOHAN should now have vcode = VCTG2_0002):
--    SELECT s.store_name, s.vcode, s.is_active, s.status
--    FROM public.stores s
--    WHERE s.store_name ilike '%calookohan%';
--    -- Expected: SM CALOOKOHAN | VCTG2_0002 | true | active
--
-- B. Vacancy linkage (unchanged — should have store_id):
--    SELECT v.vcode, v.store_id, v.store_name, v.status, v.deleted_at, v.is_archived
--    FROM public.vacancies v
--    WHERE upper(v.vcode) = 'VCTG2_0002';
--    -- Expected: VCTG2_0002 | fb83f507... | SM CALOOKOHAN | Open | null | false
--
-- C. Slot linkage (NOW should have store_id populated):
--    SELECT ps.legacy_vcode, ps.store_id, ps.slot_status
--    FROM public.plantilla_slots ps
--    WHERE upper(coalesce(ps.legacy_vcode, '')) = 'VCTG2_0002';
--    -- Expected: VCTG2_0002 | fb83f507... | open
--
-- D. Orphan count (should be 0 repairable orphans remaining):
--    SELECT count(*) as remaining_orphans
--    FROM public.plantilla_slots ps
--    LEFT JOIN public.vacancies v ON v.vcode = ps.legacy_vcode AND v.store_id IS NOT NULL
--    WHERE ps.store_id IS NULL
--      AND ps.slot_status <> 'closed'
--      AND v.store_id IS NOT NULL;
--    -- Expected: 0 (no more repairable orphans)
--
-- E. Picker output (SM CALOOKOHAN must appear):
--    SELECT store_name, already_grouped_code
--    FROM public.get_coverage_group_eligible_stores('bb02474a-3928-4383-9eee-7a3393bdef4d')
--    ORDER BY store_name;
--    -- Expected: SM CALOOKOHAN | null (available for Add Store)
--    -- Expected: EASYDAY SHOP BAGONG SILANG | null (still available)
--    -- Expected: existing assigned stores | CG-TG2-0001 (still blocked)
--
-- F. All orphan slots remaining:
--    SELECT legacy_vcode, store_id, slot_status
--    FROM public.plantilla_slots
--    WHERE store_id IS NULL AND slot_status <> 'closed';
--    -- Expected: 0 rows (all 4 production orphans repaired by §3)
