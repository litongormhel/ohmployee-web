-- OHM2026_1140 Corrective migration for Team Performance RPCs
--
-- WHY THIS EXISTS:
--   Migration 20261207000000_team_performance_report.sql was applied to Supabase
--   in its original broken state (OHM2026_1138). The file was subsequently fixed
--   locally in OHM2026_1139, but Supabase does not re-run already-applied
--   migrations. This migration applies the OHM2026_1139 fixes to the live DB.
--
-- FIXES APPLIED:
--   1. fn_team_performance_summary scope guard:
--      BEFORE: a.id = ANY(get_my_allowed_accounts())
--              → runtime error: a.id is uuid, function returns text[] of account names
--      AFTER:  a.account_name = ANY(...)
--
--   2. fn_team_performance_detail scope guard: same fix
--
-- Both functions: SECURITY DEFINER, no workflow changes, read-only.
