-- Migration: 20260612000000_fix_web_plantilla_rpc_output_safe_view.sql
-- Ticket: OHM2026_1120 - Fix Plantilla Web RPC output against live safe view rows
--
-- Root causes:
--
--   1. list_web_plantilla_employees — zero rows returned
--      The `filtered` CTE's p_active_state predicate only handled IS NULL, 'active',
--      and 'inactive'. When the Web client passes p_active_state = 'all' (or ''),
--      all three branches evaluate to FALSE and every safe-view row is excluded.
--      Fix: treat NULL, blank string, and 'all' identically as "no active-state filter".
--
--   2. list_web_plantilla_store_staffing — runtime SQL error
--      The store_keys CTE used max(account_id) and max(province_id) where both
--      columns are uuid. max(uuid) is not supported in PostgreSQL 13 and earlier
--      (no default aggregate operator for uuid) and fails with
--      "function max(uuid) does not exist".
--      Fix: replace with deterministic (array_agg(col ORDER BY col::text))[1].
--
-- The 20260611 DO-block string-patch migration attempted both fixes via
-- pg_get_functiondef() text replacement. That approach is fragile: if the stored
-- function body whitespace diverges from the expected literal, the position()
-- check returns 0 and the block raises an exception. No live Supabase apply had
-- been performed for 20260610 or 20260611, so the live database remained at the
-- 20260607 state containing both bugs.
--
-- This migration writes full CREATE OR REPLACE FUNCTION bodies. It is idempotent
-- and correct regardless of whether 20260610 / 20260611 were applied or not.
--
-- Preserved unchanged:
--   - Exact RPC signatures (parameter names, types, defaults, RETURNS TABLE columns)
--   - v_plantilla_safe as read source
--   - HC placeholder slot exclusion (source_headcount_request_id IS NULL)
--   - PII masking (no ATM raw, SSS, PhilHealth, Pag-IBIG, rate, birthdate, notes)
--   - Scope enforcement (role_level >= 90 global; otherwise user_scopes + fallback)
--   - Fail-closed auth (42501 for unauthenticated / inactive / missing profile)
--   - Row capability hints
--   - SECURITY DEFINER + SET search_path = 'public'
--   - REVOKE PUBLIC/anon + GRANT TO authenticated
--
-- Fixed behavior:
--   - list_web_plantilla_employees:
--       p_active_state = NULL  → returns all scoped safe rows (unfiltered)
--       p_active_state = ''    → returns all scoped safe rows (unfiltered)
--       p_active_state = 'all' → returns all scoped safe rows (unfiltered)
--       p_active_state = 'active'   → active lifecycle rows only
--       p_active_state = 'inactive' → inactive lifecycle rows only
--       Super Admin with all other filters NULL expects ≤ p_limit rows from
--       the 32-row v_plantilla_safe (full roster, page 1).
--   - list_web_plantilla_store_staffing:
--       Executes without UUID aggregate error on PostgreSQL 13+.
--
-- Sanity checks (run against live with a Super Admin JWT):
--
--   -- Employee roster: all 32 safe rows visible (no active-state filter)
--   SELECT count(*) FROM public.list_web_plantilla_employees(
--     p_active_state => NULL, p_limit => 200
--   );
--   -- Expected: 32
--
--   SELECT count(*) FROM public.list_web_plantilla_employees(
--     p_active_state => 'all', p_limit => 200
--   );
--   -- Expected: 32
--
--   SELECT count(*) FROM public.list_web_plantilla_employees(
--     p_active_state => '', p_limit => 200
--   );
--   -- Expected: 32
--
--   -- Store staffing: executes without error; returns ≥ 1 store row
--   SELECT count(*) FROM public.list_web_plantilla_store_staffing(
--     p_limit => 200
--   );
--   -- Expected: ≥ 1 (depends on how many stores are represented in v_plantilla_safe)
...
BEGIN
  ...
  filtered AS (
    ...
    WHERE
      ...
      AND (
        -- FIX OHM2026_1120: NULL, blank, and 'all' are all treated as "no filter".
        -- Previously only IS NULL was handled; 'all' fell through as FALSE and
        -- excluded every row.
        ...
        OR (lower(btrim(p_active_state)) = 'inactive' AND s.is_inactive)
      )
      AND (
        p_search IS NULL
...
COMMENT ON FUNCTION public.list_web_plantilla_employees(
  ...
  'OHM2026_1120 - Web presentation contract only. Treats NULL/blank/all active-state as unfiltered; derives active/inactive from v_plantilla_safe lifecycle columns. No PII exposed.';
...
BEGIN
  ...
  vacancies_scoped AS (
    ...
    WHERE
      ...
      AND (
        ...
        OR EXISTS (
          SELECT 1 FROM public.user_scopes us
          WHERE us.user_id = v_profile_id AND us.account_id = v.account_id
        )
        OR EXISTS (
          SELECT 1 FROM public.user_scopes us JOIN public.accounts ac ON ac.id = v.account_id
          WHERE us.user_id = v_profile_id AND us.account_id IS NULL AND us.group_id = ac.group_id
        )
        OR EXISTS (
  ...
  store_keys AS (
    -- FIX OHM2026_1120: max(uuid) is not supported on PostgreSQL 13 and earlier
    -- (no default aggregate operator for the uuid type). Use array_agg with a
    -- deterministic text-cast sort to pick a representative non-null UUID value.
    -- max(text) for store_name and account is valid and unchanged.
    SELECT
      u.store_id,
      max(u.store_name)                                                    AS store_name,
      (array_agg(u.account_id ORDER BY u.account_id::text) FILTER (WHERE u.account_id IS NOT NULL))[1] AS account_id,
      max(u.account)                                                       AS account_name,
      (array_agg(u.province_id ORDER BY u.province_id::text) FILTER (WHERE u.province_id IS NOT NULL))[1] AS province_id
    FROM (
...
COMMENT ON FUNCTION public.list_web_plantilla_store_staffing(
  ...
  'OHM2026_1120 - Web presentation contract only. Replaces max(uuid) in store_keys CTE with array_agg FILTER for PostgreSQL 13+ compatibility. Staffing risk thresholds are UI signals only.';
