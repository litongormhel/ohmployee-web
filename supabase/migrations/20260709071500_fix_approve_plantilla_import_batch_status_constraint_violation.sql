-- ============================================================================
-- Fix: approve_plantilla_import_batch() exception handler writes status='commit_error'
-- on failure, but 'commit_error' is not allowed by the pib_status_check constraint.
-- This mismatch causes a CHECK constraint violation that masks the actual root cause.
-- Migration: 20260709071500_fix_approve_plantilla_import_batch_status_constraint_violation.sql
-- Task: Fix pib_status_check masking real import commit errors
-- ============================================================================
...
DECLARE
  ...
  -- Normalised middle name
  v_middle_norm        text;

  -- Position_id FK resolution
  v_position_id        uuid;
...
BEGIN
  ...
  BEGIN
    ...
    LOOP
      ...
      ELSIF v_inactive_id IS NOT NULL THEN
        -- Inactive employee reactivation path
      ...
      ELSE
        -- Normal existing employee update
        UPDATE public.plantilla
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    UPDATE public.plantilla_import_batches
       SET status             = 'commit_failed', -- Fixed status here (was 'commit_error')
