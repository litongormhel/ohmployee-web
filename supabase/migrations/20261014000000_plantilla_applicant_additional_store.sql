-- OHM2026_0080: Plantilla Applicant Lookup + Direct Plantilla Assignment
--
-- Business rule: allow vacancies to source applicants from existing Plantilla
-- employees for ADDITIONAL STORE ASSIGNMENTS. This is NOT a transfer workflow.
-- The existing Plantilla employee record is preserved untouched. Approval adds a
-- new employee_store_allocations (ESA) row under the existing Plantilla record.
--
-- Normal flow:    Applicant → Confirmed Onboard → HR Emploc → Plantilla
-- Plantilla flow: Applicant(source=plantilla) → Confirmed Onboard → ESA row (direct, skips HR Emploc)
--
-- Schema changes
--   § 1  Add applicant_source, linked_plantilla_id, linked_employee_no to applicants
-- RPC
--   § 2  fn_approve_plantilla_additional_store(p_applicant_id uuid)
-- Grants
--   § 3  GRANT EXECUTE
...
ALTER TABLE public.applicants
  ADD COLUMN IF NOT EXISTS applicant_source text
    DEFAULT 'manual'
    CHECK (applicant_source IN ('manual', 'hr_emploc', 'plantilla'));

ALTER TABLE public.applicants
  ADD COLUMN IF NOT EXISTS linked_plantilla_id uuid
    REFERENCES public.plantilla(id) ON DELETE SET NULL;

ALTER TABLE public.applicants
  ADD COLUMN IF NOT EXISTS linked_employee_no text;

COMMENT ON COLUMN public.applicants.applicant_source IS
  'Tracks how this applicant record was created: manual (default), hr_emploc (sourced from HR Emploc), plantilla (sourced from an existing active Plantilla employee for an additional store assignment).';

COMMENT ON COLUMN public.applicants.linked_plantilla_id IS
  'For applicant_source=plantilla: FK to the source plantilla row from which this applicant was created. NULL for manual and hr_emploc sources.';

COMMENT ON COLUMN public.applicants.linked_employee_no IS
  'Denormalized employee_no from linked_plantilla_id. Stored for display and audit without requiring a join on the Plantilla row, which may change status over time.';

CREATE INDEX IF NOT EXISTS idx_applicants_linked_plantilla_id
  ON public.applicants (linked_plantilla_id)
  WHERE linked_plantilla_id IS NOT NULL;
...
COMMENT ON FUNCTION public.fn_approve_plantilla_additional_store(uuid) IS
  'OHM2026_0080: Approve a plantilla-sourced applicant for an additional store assignment. '
  'Inserts a new employee_store_allocations row under the existing plantilla record — no new '
  'plantilla row is created. Occupies the target slot via fn_sync_slot_to_occupied using the '
  'existing plantilla.id. Marks the applicant as Confirmed Onboard. HR Emploc bypassed. '
  'NOT a transfer workflow — no existing assignment is removed or modified.';
