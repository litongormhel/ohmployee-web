-- ============================================================
-- OHM2026_2068 — Allow Rejected Deactivation resubmission
-- Migration:  20260524117000_fix_deactivation_allow_rejected_resubmit.sql
-- Depends on: 20260524110000_deactivation_v2_rpcs.sql
--
-- Bug: create_deactivation_request rejected plantilla rows with
--   status = 'Rejected Deactivation' with:
--   "Employee must be Inactive to request deactivation. Current status: Rejected Deactivation"
--
-- Fix: restructure status check to allow both 'Inactive' (first-time
--   request) and 'Rejected Deactivation' (resubmission after rejection).
--   Existing idempotent protections are preserved:
--     • 'Pending Deactivation'  → return existing Pending row (no INSERT)
--     • 'For Deactivation'      → legacy graceful return
--     • duplicate Pending row   → pre-insert SELECT + unique_violation handler
--
-- Apply if 20260524110000 (or any prior patch) is already deployed.
