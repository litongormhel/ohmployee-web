-- Migration: 20260707100001_hrco_email_import_validation
-- Created: 2026-07-07
-- Prompt: ohm#k8d2x7qf (Plantilla Import HRCO_EMAIL column — part 2 of the HRCO
--   Employee-Level Scope Isolation feature; see 20260707100000_hrco_scope_isolation_plantilla.sql
--   for the table/RLS/RPC foundation this depends on)
-- Purpose: Add optional HRCO_EMAIL column support to the Plantilla baseline import.
--   Same validation rule as assign_hrco_to_plantilla (fn_is_valid_hrco_for_account) —
--   single code path between CSV import and manual assign, per ADR requirement.
--
--   | Case                                  | Result                          |
--   |----------------------------------------|----------------------------------|
--   | Blank                                   | Accepted — imports unassigned   |
--   | Valid email, is HRCO on this account    | Accepted — assignment created   |
--   | Email not found                         | Row BLOCKED                     |
--   | Email found, not HRCO on this account   | Row BLOCKED                     |
--
-- Smoke Tests (run after apply, capture real output before COMPLETE):
-- S1: submit a batch with 4 synthetic rows (blank / valid HRCO email / unknown email /
--     non-HRCO email) via submit_plantilla_baseline_import; inspect
--     plantilla_import_rows.validation_status + validation_errors per row.
-- S2: approve that batch (or a BEGIN...ROLLBACK dry run) and confirm exactly one
--     plantilla_hrco_assignments row is created for the valid-HRCO row, none for the
--     blank row, and the two blocked rows never reach commit.

BEGIN;

ALTER TABLE public.plantilla_import_rows
  ADD COLUMN IF NOT EXISTS hrco_email_raw text,
  ADD COLUMN IF NOT EXISTS resolved_hrco_user_id uuid;
...
-- ── approve_plantilla_import_batch: create HRCO assignment at commit time ──
