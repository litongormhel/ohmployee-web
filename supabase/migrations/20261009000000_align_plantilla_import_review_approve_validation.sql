-- ============================================================
-- OHM2026_0079 — Align Plantilla Import Review and Approve Validation
-- Migration: 20261009000000_align_plantilla_import_review_approve_validation.sql
-- Depends on:
--   20260713000000_enforce_audit_freeze.sql
--     (submit_plantilla_baseline_import — last canonical rewrite)
--   20260913000000_fix_import_inactive_reconcile_and_slot_occupancy.sql
--     (approve_plantilla_import_batch — last canonical rewrite)
--   20261006000000_enforce_archived_vcode_conflict_blocking.sql
--     (vacancy_import_batches.rollback_status, vacancies.source_vacancy_import_batch_id)
-- ============================================================
-- Purpose
-- -------
--   Corrects the classification/blocking logic in both the Review Import
--   phase (submit_plantilla_baseline_import) and the Approve phase
--   (approve_plantilla_import_batch) so that archived vs rollback-safe
--   employee/Emploc/VCode conflicts are correctly identified during Review
--   and revalidated defensively during Approve.
--
-- Business Rules Enforced
-- -----------------------
--   Active Plantilla Employee   → BLOCKED  "Employee already exists in active plantilla."
--   Archived Plantilla Employee → BLOCKED  "Employee exists in archived plantilla and cannot be reused."
--   Archived HR Emploc          → BLOCKED  "Archived Emploc cannot be reused."
--   Archived VCode              → BLOCKED  "Archived VCode cannot be reused."
--   Rollback-safe record        → ALLOWED  "Rollback record detected — eligible for re-upload."
--   QA-archived record          → BLOCKED  (qa_archive_batch_id IS NOT NULL = not rollback-safe)
--
-- Rollback-safe Detection
-- -----------------------
--   Plantilla row:
--     p.source_baseline_import_batch_id → plantilla_import_batches.id
--     batch.status = 'rolled_back' AND batch.rolled_back_at IS NOT NULL
--
--   ESA / Emploc proxy (for hr_emploc rollback-safe inference):
--     employee_store_allocations.source_import_batch_id
--       → plantilla_import_batches.id
--     batch.status = 'rolled_back'
--
--   VCode (vacancy):
--     vacancies.source_vacancy_import_batch_id IS NOT NULL
--     linked vacancy_import_batches.rollback_status = 'completed'
--     vacancies.deleted_at IS NOT NULL
--
-- What Changed vs Previous Canonical Versions
-- --------------------------------------------
--   submit_plantilla_baseline_import (vs 20260713000000):
--     + Pre-pass [E]: _pib_rollback_safe_vcodes — vacancies soft-deleted via
--       completed vacancy import rollback.
--     + Pre-pass [F]: _pib_blocked_vcodes — vacancies archived/non-reusable
--       (is_archived, status=Archived, or soft-deleted without rollback linkage).
--     ~ Employee conflict check: was FLAG ("will upsert on commit"); now:
--       1. Rollback-safe plantilla row   → ALLOWED (flag: rollback_safe_employee)
--       2. Active plantilla row          → BLOCKED  (error: active_employee)
--       3. Archived/deleted row (no rollback) → BLOCKED (error: archived_employee)
--       4. Inactive row (Inactive/Rejected Deactivation) → unchanged FLAG
--       5. No existing row               → new employee, no flag
--     + HR Emploc archived check: hr_emploc.deleted_at IS NOT NULL AND
--       no rollback-safe ESA proxy → BLOCKED (error: archived_emploc).
--       Rollback-safe via ESA → ALLOWED (flag: rollback_safe_emploc).
--     + VCode archived check (after existing active-owner check):
--       In _pib_rollback_safe_vcodes → ALLOWED (flag: rollback_safe_vcode).
--       In _pib_blocked_vcodes → BLOCKED (error: archived_vcode).
--
--   approve_plantilla_import_batch (vs 20260913000000):
--     + Defensive revalidation at start of inner BEGIN (before Phase A):
--       Scans valid/flagged rows for active-non-rollback-safe plantilla
--       conflicts and archived-non-rollback-safe VCode conflicts.
--       If any found: RAISE EXCEPTION REVALIDATION_FAILED → batch goes to
--       commit_failed with clear error message.
--     + Error humanization: ERRCODE 55000 + REVALIDATION_FAILED prefix →
--       returned verbatim (already human-readable) in commit_error_detail.
--     ~ All other logic preserved from OHM2026_0071 (20260913000000).
--
-- Preserved
-- ---------
--   New clean upload flow: UNCHANGED
--   Roving detection: UNCHANGED
--   Stationary count: UNCHANGED
--   File-level row parsing: UNCHANGED
--   Account/group/store resolution: UNCHANGED
--   Phase A store upsert in Approve: UNCHANGED
--   Phase B employee upsert paths: UNCHANGED
--   Slot occupancy wire: UNCHANGED
--   Inactive employee reconcile path: UNCHANGED
--   Deactivated employee skip: UNCHANGED
-- ============================================================


-- ============================================================
-- §1  submit_plantilla_baseline_import — Review Import
-- ============================================================
-- Full replacement of the function first defined in
-- 20260620000000_plantilla_baseline_import_v2.sql and last
-- canonically rewritten in 20260713000000_enforce_audit_freeze.sql.
-- ============================================================
...
DECLARE
  ...
  -- OHM2026_0079: employee conflict classification
  v_emp_conflict_type  text;   -- 'active_employee' | 'archived_employee' | 'rollback_safe_employee' | 'inactive_employee' | NULL
...
BEGIN
  ...
  --     Rows in _pib_rollback_safe_vcodes are implicitly excluded because:
  --       - is_archived/status=Archived are set by QA archive/manual archive,
  --         NOT by vacancy import rollback (which uses deleted_at only).
  --       - The soft-deleted non-rollback branch explicitly excludes rollback-safe rows.
  ...
  LOOP
    ...
    -- Check AFTER the active-owner check to avoid duplicate error messages.
    -- Priority: rollback-safe (ALLOWED) → archived (BLOCKED).
    -- Only fires when the VCODE is not already blocked by the active-owner check.
    ...
    THEN
      IF EXISTS (SELECT 1 FROM _pib_rollback_safe_vcodes WHERE vcode = v_vcode) THEN
        -- Rollback-safe VCode: eligible for re-upload
      ...
      ELSIF EXISTS (SELECT 1 FROM _pib_blocked_vcodes WHERE vcode = v_vcode) THEN
        -- Archived/non-reusable VCode: blocked
    ...
    ELSE
      ...
      -- ── OHM2026_0079: Employee conflict classification ─────────────────────
      -- Replaces the previous single-step "existing_employee" flag check.
      -- Priority order (first match wins):
      --   1. Rollback-safe plantilla row   → ALLOWED (flag: rollback_safe_employee)
      --   2. Active plantilla row          → BLOCKED  (error: active_employee)
      --   3. Archived/deleted non-rollback → BLOCKED  (error: archived_employee)
      --   4. Inactive row                  → ALLOWED  (flag: existing_employee, reconcile path)
      --   5. No existing row               → new employee (no flag)
      IF v_emp IS NOT NULL THEN
        v_emp_conflict_type := NULL;
        v_is_existing_emp   := false;

        -- Priority 1: rollback-safe plantilla row (any status, archived/deleted, rollback linkage)
        IF EXISTS (
          SELECT 1 FROM public.plantilla p
        ...
        -- Priority 2: active plantilla row in same or any account (same-account guard)
        ELSIF EXISTS (
          SELECT 1 FROM public.plantilla p
          WHERE p.employee_no = v_emp
            AND p.is_deleted = false
          ...
          v_errs := v_errs || jsonb_build_array(jsonb_build_object(
            ...
            'msg',           'Employee already exists in active plantilla.'
          ));
        ...
        -- Priority 4: inactive row (Inactive / Rejected Deactivation) — reconcile path
        ELSIF EXISTS (
          SELECT 1 FROM public.plantilla p
          WHERE p.employee_no = v_emp
            AND p.is_deleted = false
          ...
          v_emp_conflict_type := 'inactive_employee';
          v_is_existing_emp   := true;
          v_flags := v_flags || '[{"flag":"existing_employee","msg":"employee_no already in inactive plantilla — will reconcile on commit"}]'::jsonb;
        ...
        END IF;
      END IF;
      -- ── End OHM2026_0079 employee conflict classification ──────────────────

      -- ── OHM2026_0079: HR Emploc archived conflict blocking ─────────────────
      -- If there is a deleted hr_emploc record for this employee_no:
      --   - Rollback-safe (ESA proxy: source_import_batch_id → rolled_back batch) → ALLOWED
      --   - Otherwise → BLOCKED
      -- Only run when employee is not already fully blocked by active/archived_employee.
      -- (Rollback-safe and inactive employees still pass through; new employees too.)
      IF v_emp IS NOT NULL
         AND v_emp_conflict_type NOT IN ('active_employee', 'archived_employee')
      ...
      THEN
        IF EXISTS (
          ...
          -- Check rollback-safe via ESA proxy
          IF EXISTS (
            SELECT 1 FROM public.employee_store_allocations a
          ...
          ELSE
            -- Archived emploc without rollback linkage: blocked
            v_errs := v_errs || jsonb_build_array(jsonb_build_object(
              'field',         'EMPLOYEE_NO',
  ...
  -- OHM2026_0079: existing_employee now means "inactive employee that will
  -- be reconciled (reactivated) on commit." Active employees are BLOCKED
  -- and will not appear in valid/flagged rows.
...
COMMENT ON FUNCTION public.submit_plantilla_baseline_import IS
  'OHM2026_0079: Extends 20260713000000 (OHM2026 audit freeze gate) with '
  'correct archived vs rollback-safe conflict classification during Review Import. '
  'New behaviour: '
  '(1) Active plantilla employee → BLOCKED (was: flag only). '
  '(2) Archived/deleted non-rollback-safe plantilla employee → BLOCKED (was: not detected). '
  '(3) Rollback-safe plantilla employee (source batch rolled_back) → ALLOWED with flag. '
  '(4) Inactive employee (Inactive/Rejected Deactivation) → ALLOWED (reconcile flag, unchanged). '
  '(5) Deleted hr_emploc without rollback-safe ESA proxy → BLOCKED. '
  '(6) Rollback-safe via ESA (source_import_batch_id → rolled_back batch) → ALLOWED with flag. '
  '(7) Archived/non-reusable VCode (is_archived, status=Archived, or deleted-non-rollback) → BLOCKED. '
  '(8) Rollback-safe VCode (vacancy deleted via completed vacancy import rollback) → ALLOWED with flag. '
  'All other logic (auth, governance, audit freeze, roving detection, HC mismatch, '
  'cross-account/group checks, store resolution) preserved from 20260713000000. '
  'RBAC: Encoder, Head Admin, Super Admin only.';


-- ============================================================
-- §2  approve_plantilla_import_batch — Defensive Revalidation
-- ============================================================
-- Full replacement of the function last canonically rewritten in
-- 20260913000000_fix_import_inactive_reconcile_and_slot_occupancy.sql.
--
-- Changes from 20260913000000 (OHM2026_0071):
--   1. DECLARE: added v_reval_emp text and v_reval_vcode text for
--      the defensive revalidation pre-check.
--   2. Inside the inner exception block (before Phase A):
--      Added OHM2026_0079 defensive revalidation — scans valid/flagged
--      rows for conflicts that Review Import (submit) would now block:
--        a. Active non-rollback-safe plantilla employee conflict
--        b. Archived non-rollback-safe VCode conflict
--      If any found: RAISE EXCEPTION REVALIDATION_FAILED (ERRCODE 55000).
--   3. Exception humanization: ERRCODE 55000 + REVALIDATION_FAILED →
--      returned verbatim in commit_error_detail.
--   All other behavior (Phase A store upsert, Phase B inactive reconcile,
--   Deactivated guard, slot occupancy wire, VCODE resolution guard,
--   rollback readiness, error handling) is preserved IDENTICALLY from
--   20260913000000.
-- ============================================================
...
BEGIN
  ...
  BEGIN
    ...
    -- If conflicts are found, RAISE EXCEPTION → batch goes to commit_failed
    -- with a clear error distinguishable from late data-integrity failures.

    -- Check 1: Active non-rollback-safe plantilla employee in valid/flagged rows
    ...
    -- Check 2: Archived non-rollback-safe VCode in valid/flagged rows
    ...
    IF v_reval_vcode IS NOT NULL THEN
      RAISE EXCEPTION
        ...
        USING ERRCODE = '55000';
    END IF;
    -- ── End OHM2026_0079 defensive revalidation ────────────────────────────
    ...
    LOOP
      ...
      ELSE
        ...
        -- Defensive note (OHM2026_0079): active employees should have been
        -- BLOCKED during Review Import. If one reaches this path despite the
        -- defensive revalidation above, it means the state changed mid-commit.
        -- The UPDATE is still safe (it refreshes data without changing status),
        -- but the defensive revalidation should have caught this case first.
        UPDATE public.plantilla
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    -- OHM2026_0079: REVALIDATION_FAILED is already human-readable; return verbatim.
...
COMMENT ON FUNCTION public.approve_plantilla_import_batch IS
  'OHM2026_0079: Extends OHM2026_0071 (20260913000000) with defensive revalidation '
  'at the start of the commit block. Before Phase A (store upsert), the function '
  'scans all valid/flagged rows for two classes of conflict that Review Import now '
  'blocks: (1) active non-rollback-safe plantilla employee, (2) archived non-rollback-safe '
  'VCode. If any conflict is detected, REVALIDATION_FAILED (ERRCODE 55000) is raised, '
  'the batch moves to commit_failed, and the error message is returned verbatim as '
  'commit_error_detail — telling the operator to re-submit so Review Import can '
  'reclassify. ERRCODE 55000 / REVALIDATION_FAILED is also added to the error '
  'humanization path so it is never wrapped in the generic [state] msg format. '
  'All other logic from OHM2026_0071 is preserved: Phase A store upsert, Phase B '
  'inactive reconcile, Deactivated skip, roving override, slot occupancy wire, '
  'VCODE resolution guard, audit logging, and rollback readiness. '
